 ...
 */
export function odsekPoslov(deals: Deal[]): string {
  ...
  <h2 style="margin:16px 12px 4px;font-size:17px;color:#18181b">🎯 Top posli danes</h2>
  <p style="margin:0 12px 8px;color:#71717a;font-size:12px">Aktivni oglasi vsaj 10 % pod mediano primerljivih, razvrščeni po tem, kje je največ denarja na mizi.</p>
...
export async function preberiPosle(db: Db, koliko = 10): Promise<Deal[]> {
  try {
    const { data } = await db
      ...
      .eq("kljuc", "deal_feed")
      .maybeSingle();
    return ((data as { podatki: { deals: Deal[] } } | null)?.podatki?.deals ?? []).slice(0, koliko);
  } catch {
    return [];
  }
}
