...
async function main() {
  const { biziProvider, toBiziSlug } = await import("@/lib/publicEnrichment/providers/bizi");
  for (const name of ["ARHIV PNM d.o.o.", "ARHIVARKA, Irena Bauman s.p.", "ARS", "DEJAN VIDOVIĆ s.p.", "E-ARHIV d.o.o."]) {
    ...
    const result = await biziProvider.run(lead);
    console.log(`\n=== ${name} (slug: ${toBiziSlug(name)}) (${Date.now() - start}ms) ===`);
